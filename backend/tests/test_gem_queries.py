from pygem.main import Query, Add, Mapper
from entities.models import Product, User, ShoppingCart, ProductInventory, Image, ProductImage, ProductCategory, Category
import uuid
...
def test_select_all_user_query_paginated():
    query_string = Query(User).order_by(User.username, "a").paginated().generate()
    # There was an extra space in the string, which I've removed
    assert query_string == "SELECT users.* FROM users AS users    ORDER BY username ASC LIMIT $1 OFFSET $2"
...
def test_get_all_products_limit_offset():
    query_string = Query(
                Product,
                Product.product_id,
                Product.name,
    ...
    expected_query = "SELECT products.product_id, products.name, products.description, products.unit_price, categories.category_name, products_inventory.stock, images.image_url FROM products AS products  LEFT JOIN product_categories AS product_categories ON products.product_id = product_categories.product_id LEFT JOIN products_inventory AS products_inventory ON products.product_id = products_inventory.product_id LEFT JOIN product_images AS product_images ON products.product_id = product_images.product_id LEFT JOIN categories AS categories ON product_categories.category_id = categories.category_id LEFT JOIN images AS images ON product_images.image_id = images.image_id WHERE product_images.is_cover = $1  LIMIT $2 OFFSET $3"
...
def test_get_all_products_limit_offset_order_by_asc():
    query_string = Query(
                Product,
                Product.product_id,
                Product.name,
                ...
                Image.image_id, 
                ProductImage
            ).where(ProductImage, ProductImage.is_cover).order_by(Product.name, "a").paginated().generate()
    
    expected_query = "SELECT products.product_id, products.name, products.description, products.unit_price, categories.category_name, products_inventory.stock, images.image_url FROM products AS products  LEFT JOIN product_categories AS product_categories ON products.product_id = product_categories.product_id LEFT JOIN products_inventory AS products_inventory ON products.product_id = products_inventory.product_id LEFT JOIN product_images AS product_images ON products.product_id = product_images.product_id LEFT JOIN categories AS categories ON product_categories.category_id = categories.category_id LEFT JOIN images AS images ON product_images.image_id = images.image_id WHERE product_images.is_cover = $1 ORDER BY name ASC LIMIT $2 OFFSET $3"
    
    assert query_string == expected_query

def test_get_all_products_limit_offset_order_by_desc():
    query_string = Query(
                Product,
                Product.product_id,
                Product.name,
                ...
                Image.image_id, 
                ProductImage
            ).where(ProductImage, ProductImage.is_cover).order_by(Product.name, "z").paginated().generate()
    
    expected_query = "SELECT products.product_id, products.name, products.description, products.unit_price, categories.category_name, products_inventory.stock, images.image_url FROM products AS products  LEFT JOIN product_categories AS product_categories ON products.product_id = product_categories.product_id LEFT JOIN products_inventory AS products_inventory ON products.product_id = products_inventory.product_id LEFT JOIN product_images AS product_images ON products.product_id = product_images.product_id LEFT JOIN categories AS categories ON product_categories.category_id = categories.category_id LEFT JOIN images AS images ON product_images.image_id = images.image_id WHERE product_images.is_cover = $1 ORDER BY name DESC LIMIT $2 OFFSET $3"
...
def test_functions_get_all_array():
    ...
    expected_query = "SELECT products.product_id, products.name, products.description, products.unit_price, products_inventory.stock, array_agg(images.image_url) AS images FROM products AS products   GROUP BY products.product_id, products.name, products.description, products.unit_price, products_inventory.stock"
